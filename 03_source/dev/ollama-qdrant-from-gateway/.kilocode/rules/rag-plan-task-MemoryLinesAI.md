...
- Планируем: Добавить Prometheus-метрики для мониторинга
...
### Возможности которые добавим после текущей задачи - метрики и мониторинг

-- **Prometheus-метрики** (для мониторинга работы Gateway):
  ...
  - `rag_inference_duration_seconds` — гистограмма времени инференса

-- **Дополнительные улучшения**:
  - Health check endpoint с информацией о состоянии Redis
  - Метрики использования памяти Redis
  - Логирование ошибок с stack trace в JSON формате
...
*>* Реализация CacheLogger.cs (✅ выполнено)
*>>* Статический класс с методами Hit/Miss/Coalesced (✅ выполнено)
*>>* Переключатель формата через конфигурацию (✅ выполнено)
*>>* Форматирование размера B/KB/MB (✅ выполнено)
*>>* Потокобезопасность через volatile (✅ выполнено)

*>* Интеграция в Gateway (✅ выполнено)
*>>* /api/embed — замена `Console.WriteLine` на `CacheLogger.*` (✅ выполнено)
*>>* /api/embeddings — аналогично (✅ выполнено)
*>>* /v1/embeddings — аналогично + замер длительности через `Stopwatch` (✅ выполнено)

*>* Обновление appsettings.json (✅ выполнено)
*>>* Добавить секцию `"Logging": { "CacheFormat": "text" }` (✅ выполнено)
*>>* Документировать переключение формата (✅ выполнено)

*>* Предложения по обновлению .kilocode\rules\rag-plan.md (✅ выполнено - включено в текущий файл)
*>>* Зафиксировано: поддержка двух Ollama embedding API (`/api/embed`, `/v1/embeddings`)
*>>* Зафиксировано: механизм кэш-ожидания (polling) как штатный паттерн
*>>* Зафиксировано: модель `qwen3-embedding:0.6b-q8_0` требует `BatchSize ≤ 10` на CPU N100
*>>* Зафиксировано: сервис логирования кэша `CacheLogger` с поддержкой text/json
